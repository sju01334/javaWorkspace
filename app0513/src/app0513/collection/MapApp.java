...
//		System.out.println(map.get("fruit3"));
		...
		//주의: 아래의 keySet 메서드는 map에 들어있는 value가 아닌, 그 value와 쌍을 이루는 key값만을 추출한다
		Set<String> set = map.keySet(); //fruit1, fruit2, fruit3가 순서없이 모여있게 됨
		
		Iterator<String> it = set.iterator(); //이름을 일렬로 늘어서게 함
		
		while(it.hasNext()) {
			String key = it.next(); //키를 얻었기 때문에 이 시점에 Map에 들어있는 데이터를 검색할 수 있따
			String value = map.get(key); //키에 연결된 value 변환
